...
var AUTH_URL = "https://localhost:3001/gateway/oauth/token";
var MCP_URL = "https://localhost:3001/gateway/mcp";
var OAuthCredentialProvider = class {
  token = null;
  clientId;
  clientSecret;
  authUrl;
  ...
  isTokenExpired() {
    ...
    const now = Date.now();
    const expirationTime = this.token.acquired_at + this.token.expires_in * 1e3;
    return now >= expirationTime - 3e5;
  }
  async fetchNewToken() {
...
var RemoteMCPConnector = class {
  constructor(clientId, clientSecret, authUrl, mcpUrl) {
    this.mcpUrl = mcpUrl;
    this.oauthProvider = new OAuthCredentialProvider(clientId, clientSecret, authUrl);
    this.client = new Client(
      { name: "mcp-express-cli", version: "1.0.0" },
      { capabilities: {} }
    );
  }
  oauthProvider;
  ...
  async ensureValidConnection() {
    const currentToken = await this.oauthProvider.getAccessToken();
    if (this.lastTokenUsed !== currentToken) {
      const baseUrl = new URL(this.mcpUrl);
      const transport = new StreamableHTTPClientTransport(baseUrl, {
        requestInit: {
          headers: {
            "Authorization": await this.oauthProvider.getAuthorizationHeader()
          }
        }
      });
...
async function main() {
  console.error("Starting MCP Express CLI...");
  const mcpConnector = new RemoteMCPConnector(CLIENT_ID, CLIENT_SECRET, AUTH_URL, MCP_URL);
  console.error("Connecting to MCP server...");
  await mcpConnector.connect();
  console.error("Connected to MCP server.");
  const version = mcpConnector.getClient().getServerVersion();
  if (!version) {
    console.error("Failed to connect to MCP server. Exiting.");
    (0, import_process.exit)(1);
  }
  console.error(`Connected to MCP server: ${version.name} v${version.version}`);
  const server = new Server(
    {
      name: version.name || "mcp-express-cli",
  ...
  function setupToolHandlers(server2, mcpConnector2) {
    server2.setRequestHandler(ListToolsRequestSchema, async () => {
      const tools = await mcpConnector2.listTools();
      return {
        tools
      };
    });
    server2.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const result = await mcpConnector2.callTool(name, args);
      return {
        content: result
      };
    });
  }
  setupToolHandlers(server, mcpConnector);
  function setupToolChangeNotificationTimer(server2) {
    const TOOL_CHANGE_INTERVAL = 3e5;
    setInterval(() => {
      try {
        ...
        console.error("Sent tools/list_changed notification");
      } catch (error) {
        console.error("Failed to send tools/list_changed notification:", error);
      }
    }, TOOL_CHANGE_INTERVAL);
    console.error(`Tool change notification timer started (interval: ${TOOL_CHANGE_INTERVAL}ms)`);
  }
  setupToolChangeNotificationTimer(server);
  console.error("MCP Express CLI is ready to accept requests.");
  const transport = new StdioServerTransport();
  await server.connect(transport);
}
main().catch(console.error);
console.error("MCP Express CLI exited.");
