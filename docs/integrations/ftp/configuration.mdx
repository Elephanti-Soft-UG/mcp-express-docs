...
Once FTP access is validated, configuration defines how MCP Express exposes FTP file operations as callable tools. Most actions require a remote path and may optionally include filters, file content, or local destinations depending on the tool.
...
| Remote File Path | Some | Remote file path for `read`, `write`, `upload`, or `download`. |
| Local Path | Some | Local file path for `upload` and `download` (on your MCP server). |
