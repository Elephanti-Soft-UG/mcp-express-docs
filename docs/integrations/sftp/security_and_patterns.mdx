...
The tool accepts Remote Path, Remote File Path, Local Path, and File Content.
...
Unvalidated path templates can point the tool at unintended directories or files. Enforce allowlists, root-prefix checks, and input validation before allowing runtime substitution.
:::
