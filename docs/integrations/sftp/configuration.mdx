...
Use action configuration to define which file operation the tool performs and which parameters are resolved at runtime. Paths and content fields support Jinja2 templates (for example, `/data/{{ year }}/{{ month }}/report.csv`, which inserts the provided `year` and `month` values into the path at runtime).
