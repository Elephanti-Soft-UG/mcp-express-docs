...
| Authentication Method | IAM User keys, STS session credentials, or environment-backed role authentication. | Yes |
...
| AWS Session Token | Temporary token required for STS-based credentials. | Conditional |
| Request Timeout | Max duration (seconds) before the connector aborts an upstream request. | Optional (Default: 30) |
