...
Integrate MCP Express with PostgreSQL databases to create tools that can query and interact with your data. This integration allows you to execute SQL queries against your PostgreSQL database and use the results in your MCP tools.
...
|-----------|----------|-------------|
| host | Yes | IP address or URL of the PostgreSQL server |
...
1. **Select PostgreSQL Integration**: In your MCP server dashboard, choose "PostgreSQL" from the available integrations
2. **Configure Connection**: Enter your database host, database name, username, and password
3. **Set Port and Timeout**: Optionally customize the port (default: 5432) and connection timeout (default: 10 seconds)
...
![PostgreSQL Configuration](images/database-config.png)

## Query Templates

The PostgreSQL integration uses templating to create dynamic SQL queries based on tool inputs. This allows you to safely parameterize your queries while maintaining flexibility.
...
The PostgreSQL integration can execute fixed SQL queries, useful for performing predefined tasks. For example, counting the number of users living in postal code 71250:

```sql
SELECT COUNT(*) as user_count
FROM users
WHERE postal_code='71250'
```

#### Dynamic Template Queries

The PostgreSQL integration supports parameterized queries that accept user input. For example, counting users in a postal code specified by the user:
```sql
SELECT COUNT(*) as user_count
FROM users
...
The PostgreSQL integration can accept complete SQL queries generated by the LLM. **Important:** Carefully evaluate all security risks, as the LLM may generate incorrect or potentially harmful queries based on user input.
...
- Check the pg_hba.conf file for proper authentication method configuration
...
- Examine PostgreSQL error logs for detailed diagnostic information
