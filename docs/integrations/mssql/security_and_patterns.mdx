...
- **Row-Level Security (RLS)**: We recommend provisioning a MS SQL Server user with appropriate security policies. This ensures that even if an LLM generates a broad query, it can only access rows permitted by its database role.
