...
:::tip
For a conceptual explanation of what a static egress IP is and how it fits the MCP architecture, see [Static Egress IP](../advanced/static-ip-address) in the Advanced section.
:::
...
| Property | Value |
|---|---|
| IP Address | [See Static Egress IP](../advanced/static-ip-address) |
| Type | IPv4 |
...
The exact steps depend on your infrastructure. In general, you need to add an inbound rule that permits TCP traffic from the [MCP Express IP address](../advanced/static-ip-address) on the port your service listens on.
...
To enable MCP Express to connect to a resource that enforces IP-based access control, add the [MCP Express IP address](../advanced/static-ip-address) to your inbound allowlist on the relevant port. Pair this with strong authentication and TLS to maintain a layered security posture.

For questions or connectivity issues, contact our support team at [MCP Express Support](https://app.mcp-express.com/support).
